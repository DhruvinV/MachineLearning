...
def AffineBackward(grad_y, h, w):
    ...
    """
    grad_h = np.matmul(grad_y,np.transpose(w))
    grad_w = np.matmul(np.transpose(h),grad_y)
...
def NNUpdate(model, eps, momentum):
    ...
    model['W1'] -= eps*model['v1']
    model['W2'] -= eps*model['v2']
    model['W3'] -= eps*model['v3']
    model['b11'] = ((momentum*model['b11'])+((1-momentum)*model['dE_db1']))
    model['b22'] = ((momentum*model['b22'])+((1-momentum)*model['dE_db2']))
    model['b33'] = ((momentum*model['b33'])+((1-momentum)*model['dE_db3']))
    model['b1'] -= eps*model['b11']
    model['b2'] -= eps*model['b22']
    model['b3'] -= eps*model['b33']
...
def Train(model, forward, backward, update, eps, momentum, num_epochs,
    ...
    inputs_train, inputs_valid, inputs_test, target_train, target_valid, \
        target_test = LoadData('/Users/dhruvin/Desktop/Fall_2019/CSC311/q3_materials/toronto_face.npz')
   ...
   #####
    DisplayPlot(train_acc_list, valid_acc_list, str(num_hiddens[0])+"_"+str(num_hiddens[1])+"_"+"acc", number=1)
    DisplayPlot(train_ce_list, valid_ce_list, str(num_hiddens[0])+"_"+str(num_hiddens[1])+"_"+"CE", number=0)
...
def Evaluate(inputs, target, model, forward, batch_size=-1):
    ...
    plot_uncertain_images(x, t, prediction, threshold=0.5)
    ce /= num_cases
    acc /= num_cases
    return ce, acc
...
def main():
    ...
    # for batch_size in np.linspace(1,1000,5):
    # Initialize model.
    ...
    model,stats = Train(model, NNForward, NNBackward, NNUpdate, eps,
                    momentum, num_epochs, int(batch_size),num_hiddens)
...
if __name__ == '__main__':
    ...
    #     'valid_acc': valid_acc_list
    # }
    # x = ["bacth_size_1000.0.npz","bacth_size_1.0.npz","bacth_size_250.75.npz","bacth_size_500.5.npz","bacth_size_750.25.npz"]
    # j =  0
    # for i in x:
    #     acc = "Acc"+str(j)
    #     ce = "CE"+str(j)
    #     file = np.load(i)
    #     DisplayPlot(file["train_acc"], file['valid_acc'], acc, number=1)
    #     DisplayPlot(file['train_ce'], file['valid_ce'], ce, number=0)
    #     j = j+1
